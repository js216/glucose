...
static void put(uint32_t *fb, int stride, int fbw, int fbh,
...
static void dot(uint32_t *fb, int stride, int fbw, int fbh,
...
{
    if (glu < PLOT_GLU_MIN) glu = PLOT_GLU_MIN;
    if (glu > PLOT_GLU_MAX) glu = PLOT_GLU_MAX;
    return y + h - 2 - (int)((long)(h - 3) * (glu - PLOT_GLU_MIN)
                             / (PLOT_GLU_MAX - PLOT_GLU_MIN));
}
...
void plot_render(uint32_t *fb, int stride, int fbw, int fbh,
                 int x, int y, int w, int h,
                 const plot_pt *pts, int npts, long now, int hours,
                 int radius, uint32_t (*color)(int glu),
                 int hi_idx, uint32_t hi_color)
{
    const uint32_t frame = 0xFF555555;   /* 50/300 reference lines + sides   */
    ...
    if (span <= 0 || w < 4 || h < 4) return;

    int y50 = glu_to_y(50, y, h), y300 = glu_to_y(300, y, h);

    /* faint shade behind the 70-180 in-range band */
    int y_hi = glu_to_y(180, y, h), y_lo = glu_to_y(70, y, h);
    ...
    for (long ts = gstep; ts <= span; ts += gstep) {
        ...
        for (int j = y300 + 1; j < y50; j++) put(fb, stride, fbw, fbh, gx, j, vgrid);
        for (int j = y50 - 2 * radius; j <= y50; j++) put(fb, stride, fbw, fbh, gx, j, vtick);
    }

    /* gray reference lines at the 50 and 300 bounds, plus vertical sides */
    for (int i = 0; i < w; i++) {
        put(fb, stride, fbw, fbh, x + i, y50,  frame);
        put(fb, stride, fbw, fbh, x + i, y300, frame);
    }
    for (int j = y300; j <= y50; j++) {
    ...
    for (int i = 0; i < npts; i++) {
        ...
        if (i == hi_idx) { hx = px; hy = py; continue; }
        dot(fb, stride, fbw, fbh, px, py, radius, color(pts[i].glu));
    }
    if (hx >= 0) {
        /* white vertical marker; only the dot itself is highlighted in colour */
        for (int j = y300 + 1; j < y50; j++) put(fb, stride, fbw, fbh, hx, j, 0xFFFFFFFF);
        dot(fb, stride, fbw, fbh, hx, hy, radius + 2, hi_color);
    }
}

int plot_point_xy(int x, int y, int w, int h,
                  plot_pt p, long now, int hours, int *ox, int *oy)
{
    long span = (long)hours * 3600;
    if (span <= 0) return 0;
    long dt = now - p.t;
...
int plot_hit(int x, int y, int w, int h,
             const plot_pt *pts, int npts, long now, int hours,
             int tx, int ty)
{
    ...
    for (int i = 0; i < npts; i++) {
        ...
        if (ddx < 0) ddx = -ddx;
        if (best < 0 || ddx < bestd) { best = i; bestd = ddx; }
    }
    return best;
}
