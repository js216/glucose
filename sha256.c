...
void sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    ...
    size_t rem = len - i;
    for (size_t n = 0; n < rem; n++) block[n] = data[i + n];
    block[rem] = 0x80;
    for (size_t n = rem + 1; n < 64; n++) block[n] = 0;
    if (rem >= 56) { compress(h, block); for (int n = 0; n < 64; n++) block[n] = 0; }
    uint64_t bitlen = (uint64_t)len * 8;
    for (int n = 0; n < 8; n++) block[56 + n] = (uint8_t)(bitlen >> (56 - 8*n));
    compress(h, block);
    for (int t = 0; t < 8; t++) {
        out[t*4]   = h[t] >> 24; out[t*4+1] = h[t] >> 16;
        out[t*4+2] = h[t] >> 8;  out[t*4+3] = h[t];
    }
}
